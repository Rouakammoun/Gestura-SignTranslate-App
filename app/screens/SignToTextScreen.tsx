...
const SignToTextScreen = () => {
    ...
    const [translatedText, setTranslatedText] = useState(t('translated_text_placeholder'));
    const [cameraType] = useState<'back' | 'front'>('back');
    const [permission, requestPermission] = useCameraPermissions();
    const cameraRef = useRef(null);
    ...
    const speakText = async () => {
        try {
            ...
            console.error(t('audio_error'), error);
        }
    };
    ...
    if (!permission.granted) {
        return (
            <View style={styles.permissionContainer}>
                <Text style={styles.permissionText}>{t('camera_permission_request')}</Text>
    ...
    return (
        ...
        >
            ...
            <View style={styles.textContainer}>
                ...
                <View style={[styles.buttonsContainer, dynamicStyles.actionButtons]}>
                    <TouchableOpacity onPress={translateText} style={styles.actionButton}>
                        <Ionicons name='language' size={30} color='#003C47' />
                        <Text style={styles.buttonLabel}>{t('translate')}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={speakText} style={styles.actionButton}>
                        <Ionicons name='volume-high' size={30} color='#003C47' />
                        <Text style={styles.buttonLabel}>{t('speak')}</Text>
                    </TouchableOpacity>
                </View>
            </View>
...
// Keep the same StyleSheet.create from previous example
